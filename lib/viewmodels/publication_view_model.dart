...
class PublicationViewModel extends ChangeNotifier {
  ...
  String? _lastTopicSuggestionQuery;
  final Map<String, List<ResearchTopic>> _topicSuggestionCache = {};
  ...
  String _currentWorkQuery = '';
  List<String> _currentWorkTopicIds = [];
  String? _workSearchDashboardKey;
  ...
  String get currentWorkQuery => _currentWorkQuery;
  List<String> get currentWorkTopicIds =>
      List.unmodifiable(_currentWorkTopicIds);
  bool get hasActiveWorkSearch => _currentWorkQuery.isNotEmpty;
  String get currentWorkScopeLabel {
    if (_currentWorkQuery.isEmpty) return '';
    if (_selectedTopics.isEmpty && _currentWorkTopicIds.isEmpty) {
      return _currentWorkQuery;
    }
    final topicLabel = _selectedTopics.isEmpty
        ? '${_currentWorkTopicIds.length} topic filters'
        : _selectedTopics.map((topic) => topic.name).join(', ');
    return '$_currentWorkQuery - $topicLabel';
  }
  ...
  Future<void> loadTopicSuggestions(String query) async {
    ...
    if (trimmedQuery.length < 2) {
      ...
      _lastTopicSuggestionQuery = null;
      _topicSuggestions = [];
    ...
    if (_lastTopicSuggestionQuery == cacheKey &&
        !_isLoadingTopicSuggestions &&
        (_topicSuggestions.isNotEmpty || _topicSuggestionError.isNotEmpty)) {
      return;
    }

    final cachedTopics = _topicSuggestionCache[cacheKey];
  ...
  Future<bool> autoSelectTopicsForQuery(String query, {int limit = 3}) async {
    final trimmedQuery = query.trim();
    if (trimmedQuery.length < 2) return false;

    await loadTopicSuggestions(trimmedQuery);
    if (_topicSuggestions.isEmpty) return false;

    _selectedTopics = _topicSuggestions.take(limit).toList();
    _lastSearchText = _selectedTopics.map((topic) => topic.name).join(', ');
    notifyListeners();
    return _selectedTopics.isNotEmpty;
  }

  Future<void> loadWorkSearchDashboard(String query) async {
    final trimmedQuery = query.trim();
    ...
    if (_hasTestingFixtures) {
      ...
      _lastSearchText = trimmedQuery;
      _currentWorkQuery = trimmedQuery;
      _currentWorkTopicIds = topicIds;
      _currentTopic = topicLabel.isEmpty
          ? trimmedQuery
          : '$trimmedQuery - $topicLabel';
      _currentTopicIds = [];
      _isLoadingTopicDashboard = false;
      _topicDashboardError = '';
      _workSearchDashboardKey =
          '${trimmedQuery.toLowerCase()}|${_topicKey(topicIds) ?? ''}';
      notifyListeners();
      return;
    }
    ...
    final requestId = ++_searchRequestId;
    final topicLabel = _selectedTopics.map((topic) => topic.name).join(', ');
    final scopeLabel = topicLabel.isEmpty
        ? trimmedQuery
        : '$trimmedQuery - $topicLabel';
    ...
    _workSearchDashboardKey = searchKey;
    _topicDashboardTotalWorks = 0;
    ...
    try {
      ...
      final trends = await _apiService
          .getWorkSearchPublicationTrend(
            query: trimmedQuery,
            topicIds: topicIds,
          )
  ...
  void toggleTopicFilter(ResearchTopic topic) {
    _toggleSelectedTopic(topic);
    notifyListeners();
  }

  void _toggleSelectedTopic(ResearchTopic topic) {
  ...
  void setSelectedTopics(List<ResearchTopic> topics) {
    setSelectedTopicsForSearch(topics);
  }

  void setSelectedTopicsForSearch(
